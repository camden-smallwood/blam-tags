...
pub(crate) fn build_identity_multipart_bytes() -> (Vec<u8>, Vec<u8>) {
    ...
    // slope=1, offset=0.
    let mut compact = Vec::with_capacity(20);
    compact.extend_from_slice(&1i32.to_le_bytes()); // function_count
    compact.push(4); // part type = Linear
    compact.extend_from_slice(&[0, 0, 0]); // pad
    compact.extend_from_slice(&f32::MAX.to_le_bytes()); // ending_x (last segment)
    compact.extend_from_slice(&1.0f32.to_le_bytes()); // slope
    compact.extend_from_slice(&0.0f32.to_le_bytes()); // offset

    // Editor: 148 bytes = [count:i32][4 × 36-byte parts]. Part =
    // [type, corner_l, corner_r, pad][4 × real_point2d]. Segment 0 is a
    // Linear (0,0)→(1,1); remaining points/parts stay zero.
    let mut editor = vec![0u8; 148];
    editor[0..4].copy_from_slice(&1i32.to_le_bytes()); // count
    editor[4] = 4; // part 0 type = Linear
    editor[8..12].copy_from_slice(&0.0f32.to_le_bytes()); // p0.x
    editor[12..16].copy_from_slice(&0.0f32.to_le_bytes()); // p0.y
    editor[16..20].copy_from_slice(&1.0f32.to_le_bytes()); // p1.x
    editor[20..24].copy_from_slice(&1.0f32.to_le_bytes()); // p1.y
    (compact, editor)
}
...
pub struct TagFunction {
    ...
    /// clear — every shipped MCC `mapping_function` carries it. Layout
    /// (verified vs Reach `byte_swap_function_definition @0x82e8d2c8`):
    /// `header[32] + compact[compact_size] + editor_trailer`, where the
    /// trailer is `graph_count` fixed-size editor structs (transition 12,
    /// periodic 20, exponent 12, multi_part 148). For transition/periodic/
    /// exponent the editor bytes MIRROR the compact; only multi_part has a
    /// distinct richer editor (control points + corner flags). Preserved
    /// verbatim on round-trip so a load→save of an untouched function is
    /// byte-identical; regenerated by `TagFunctionEditor` on edit.
    editor_data: Vec<u8>,
    /// The exact bytes this function parsed from. Returned verbatim by
    /// [`Self::to_bytes`] until a mutator sets `dirty`, guaranteeing a
    /// byte-identical round-trip for unmodified functions.
    raw: Vec<u8>,
    dirty: bool,
}
...
impl TagFunction {
    ...
    ///
    /// `compact_size` (header bytes 28..32) is the TOTAL compact size across
    /// all graphs (= offset to the editor trailer), NOT the per-graph size.
    /// A RANGE-flagged non-Constant function holds two compacts back-to-back;
    /// the second begins right after the first graph's compact.
    pub fn parse(data: &[u8]) -> Result<Self, TagFunctionError> {
        let kind = FunctionKind::parse(data)?;
        let header = kind.header();
        let compact_total = header.compact_size.max(0) as usize;
        let ftype = header.function_type;
        let ranged = header.flags.is_ranged()
            && ftype != FunctionType::Constant
            && ftype != FunctionType::Identity;
        // The second graph's compact starts right after the first graph's
        // compact (self-delimiting size, not `compact_size`, which is the
        // total across both graphs).
        let first_len = kind.compact_to_bytes().len();
        let ranged_second = if ranged {
            FunctionKind::parse_at(data, 32 + first_len)
                .ok()
                .map(Box::new)
        } else {
            None
        };
        // Editor trailer follows the whole compact region — but only
        // transition/periodic/exponent/multi_part carry one (and only when
        // not OPTIMIZED). Other types (identity/constant, and the legacy
        // linear/spline/linear-key forms) have no trailer.
        ...
        ) && !header.flags.is_optimized();
        let editor_data = if has_trailer {
            let editor_off = 32 + compact_total;
            data.get(editor_off..).unwrap_or(&[]).to_vec()
        } else {
            Vec::new()
        };
        Ok(Self {
    ...
    // -- Foundation editor query surface (ports `c_function_definition`
    //    getters; see `blam-tags-function-editor-requirements.md`). --

    /// Whether the RANGE flag is set (a second graph is present). Mirrors
    /// `c_function_definition::get_ranged @0x82e892d8` = the RANGE bit.
    pub fn is_ranged(&self) -> bool {
        self.header().flags.is_ranged()
    }

    /// Number of graph slots: 2 when ranged, else 1. Constant/Identity carry
    /// no compact so their "second graph" is empty even when RANGE is set.
    /// Mirrors `c_function_definition::get_graph_count @0x82e8c8a8`.
    pub fn graph_count(&self) -> usize {
        1 + self.is_ranged() as usize
    }

    /// The function type of graph `slot` (0 = primary, 1 = ranged second).
    /// Both graphs always share the header type; returns `None` for an
    /// out-of-range slot or a slot-1 query on an unranged function.
    pub fn graph_kind(&self, slot: usize) -> Option<FunctionType> {
        match slot {
            0 => Some(self.function_type()),
            1 if self.ranged_second.is_some() => Some(self.function_type()),
            _ => None,
        }
    }

    /// Internal per-slot access to the decoded curve (0 = primary, 1 = ranged
    /// second). Used by [`TagFunctionEditor`] to edit both graphs uniformly.
    pub(crate) fn graph(&self, slot: usize) -> Option<&FunctionKind> {
        match slot {
            0 => Some(&self.kind),
            1 => self.ranged_second.as_deref(),
            _ => None,
        }
    }

    pub fn is_clamped(&self) -> bool { self.header().flags.is_clamped() }
    pub fn is_cyclic(&self) -> bool { self.header().flags.is_cyclic() }
    pub fn is_exclusion(&self) -> bool { self.header().flags.has_exclusion() }
    pub fn exclusion_min(&self) -> f32 { self.header().exclusion_min }
    pub fn exclusion_max(&self) -> f32 { self.header().exclusion_max }

    /// Number of populated color slots (0 = scalar). Mirrors
    /// `c_function_definition::get_color_count @0x82e8c8e8`: the
    /// `color_graph_type` byte, except a non-ranged Constant color forces 1.
    pub fn color_count(&self) -> usize {
        let cgt = self.color_graph_type() as usize;
        if cgt == 0 {
            return 0;
        }
        if self.function_type() == FunctionType::Constant && !self.is_ranged() {
            1
        } else {
            cgt
        }
    ...
    /// Raw editor-data trailer bytes preserved from parse (empty when the
    /// function is OPTIMIZED). [`TagFunctionEditor`] reads this to recover the
    /// exact multi_part control points instead of reverse-fitting compacts.
    pub(crate) fn editor_data(&self) -> &[u8] {
        &self.editor_data
    }

    /// The 32-byte header, serialized (preserves flags, color graph type,
    /// colors, exclusion, clamp range). Used by [`TagFunctionEditor`] to
    /// carry unrelated header bytes across a rebuild.
    pub(crate) fn header_bytes(&self) -> [u8; 32] {
        self.header().to_bytes()
    }

    /// On-disk compact bytes for graph `slot` (0 = primary, 1 = ranged
    /// second). Used by [`TagFunctionEditor`] to rebuild typed-compact edits.
    pub(crate) fn graph_compact_bytes(&self, slot: usize) -> Option<Vec<u8>> {
        self.graph(slot).map(|k| k.compact_to_bytes())
    }
    ...
    /// Serialize back to the raw `mapping_function` `data` blob:
    /// `header + compact(s) + editor-data trailer` (matching [`Self::parse`]).
    ///
    /// An unmodified function returns its original bytes verbatim (byte-exact
    /// round-trip). After an edit it rebuilds the header + compact(s) and
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        ...
        // Recompute the total compact size in the serialized header so the
        // editor trailer offset stays correct after an edit changed the type.
        let compact_total = out.len() - 32;
        out[28..32].copy_from_slice(&(compact_total as i32).to_le_bytes());
        // Re-append the editor trailer (unless OPTIMIZED, which has none).
        if !self.header().flags.is_optimized() {
            let trailer = self.editor_trailer_bytes();
            out.extend_from_slice(&trailer);
        }
        out
    }

    /// The editor-data trailer to serialize. For transition/periodic/exponent
    /// the on-disk editor struct is byte-identical to the compact, so it is
    /// regenerated from the compact(s) to remain consistent after an edit.
    /// For multi_part (and anything else) the raw `editor_data` preserved at
    /// parse — or set by [`TagFunctionEditor`] — is used verbatim.
    fn editor_trailer_bytes(&self) -> Vec<u8> {
        match self.function_type() {
            FunctionType::Transition | FunctionType::Periodic | FunctionType::Exponent => {
                let mut t = self.kind.compact_to_bytes();
                if let Some(second) = &self.ranged_second {
                    t.extend_from_slice(&second.compact_to_bytes());
                }
                t
            }
            _ => self.editor_data.clone(),
        }
    }

    /// Mark the function as edited so [`Self::to_bytes`] rebuilds rather than
    /// returning the pristine parsed bytes.
...
mod tests {
    ...
    /// Golden ranged-periodic `mapping_function` from
    /// `levels/solo/100_citadel/shaders/panel_elevator.shader`:
    /// header(32) + 2×20 periodic compacts + 2×20 periodic editors = 112B.
    fn golden_ranged_periodic() -> Vec<u8> {
        hexvec(
            "03 25 02 00 25 19 00 00 00 00 00 00 00 00 00 00 0e 0b 00 00 00 00 00 00 00 00 00 00 28 00 00 00 \
             02 cd cd cd 00 00 80 3f 00 00 00 00 00 00 00 00 00 00 80 3f \
             00 cd cd cd 00 00 80 3f 00 00 00 00 00 00 00 00 00 00 80 3f \
             02 cd cd cd 00 00 80 3f 00 00 00 00 00 00 00 00 00 00 80 3f \
             00 cd cd cd 00 00 80 3f 00 00 00 00 00 00 00 00 00 00 80 3f",
        )
    }
    ...
    fn golden_ranged_periodic_parses_and_roundtrips_byte_exact() {
        ...
        assert_eq!(f.color_count(), 2); // TwoColor
        assert_eq!(f.header().compact_size, 40); // TOTAL over both graphs
        assert_eq!(f.editor_data().len(), 40); // 2×20 periodic editors
        // Unmodified → byte-identical round-trip (raw preserved).
        assert_eq!(f.to_bytes(), data);
    }

    #[test]
    fn edited_periodic_rebuilds_with_editor_trailer_not_truncated() {
    ...
    #[test]
    fn unranged_periodic_roundtrip_is_not_truncated() {
        // 32 header + 20 compact + 20 editor = 72 (the old code truncated to 52).
        let mut data = vec![0u8; 32];
        data[0] = FunctionType::Periodic as u8;
        data[28..32].copy_from_slice(&20i32.to_le_bytes());
        data.extend_from_slice(&[2, 0xcd, 0xcd, 0xcd]); // function_index=2 + pad
        data.extend_from_slice(&1.0f32.to_le_bytes()); // frequency
        data.extend_from_slice(&0.0f32.to_le_bytes()); // phase
        data.extend_from_slice(&0.0f32.to_le_bytes()); // amp_min
        data.extend_from_slice(&1.0f32.to_le_bytes()); // amp_max
        ...
        f.set_clamp_range(0.0, 1.0);
        assert_eq!(f.to_bytes().len(), 72);
    }
